...
export interface EnhancedCryptoCardProps {
  cryptoId: string
  index: number
}
...
function EnhancedCryptoCard({ cryptoId, index }: EnhancedCryptoCardProps) {
  const { markets, isLoading: marketsLoading } = useCryptoMarkets('usd', [cryptoId], 1)
  const { history, isLoading: historyLoading } = useCryptoHistory(cryptoId, 'usd', 7)

  const isLoading = marketsLoading || historyLoading
  ...
  const formattedPrice = useMemo(() => {
    if (!crypto) return ''
    return formatPrice(crypto.currentPrice)
  }, [crypto?.currentPrice])

  const formattedMarketCap = useMemo(() => {
    if (!crypto?.marketCap) return ''
    return formatLargeNumber(crypto.marketCap)
  }, [crypto?.marketCap])

  const formattedVolume = useMemo(() => {
    if (!crypto?.totalVolume) return ''
    return formatLargeNumber(crypto.totalVolume)
  }, [crypto?.totalVolume])

  const formattedPriceChange = useMemo(() => {
    if (!crypto) return ''
    return formatPrice(Math.abs(crypto.priceChange24h))
  ...
  if (isLoading) {
    return (
      <motion.div
        ...
        className="relative h-36 rounded-3xl bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-900 animate-pulse"
      />
    )
  }
              ...
              <div className="flex items-center justify-end mb-1 ml-2">
                ...
                >
                  ...
                  <motion.path
                    ...
                    fill={`url(#crypto-gradient-${cryptoId})`}
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
...
export default memo(EnhancedCryptoCard, (prevProps, nextProps) => {
  // Only re-render if cryptoId or index changes
  return prevProps.cryptoId === nextProps.cryptoId && prevProps.index === nextProps.index
})
